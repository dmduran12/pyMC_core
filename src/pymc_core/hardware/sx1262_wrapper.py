...
class SX1262Radio(LoRaRadio):
    ...
    async def _handle_transmission_timeout(self, timeout_seconds: float, start_time: float) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to read IRQ status during timeout handling: {e}")
        
        # Increment error counter for watchdog
        self._consecutive_errors += 1
        
        # If this is a repeated issue, trigger recovery
        if self._consecutive_errors >= self._max_consecutive_errors:
            logger.error("[TX Timeout] Multiple TX timeouts - triggering radio recovery")
